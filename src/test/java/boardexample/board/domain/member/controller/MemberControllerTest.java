package boardexample.board.domain.member.controller;
    ...
    public void 비밀번호수정_실패_검증비밀번호가_틀림() throws Exception {
        ...
        mockMvc.perform(
                        put("/member/password")
                                ...
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(updatePassword))
                //.andExpect(status().isOk());//TODO 테스트코드 수정
                .andExpect(status().isBadRequest());//TODO 테스트코드 수정
    ...
    public void 회원정보조회_실패_없는회원조회() throws Exception {
        ...
        MvcResult result = mockMvc.perform(
                        get("/member/2211")
                                .characterEncoding(StandardCharsets.UTF_8)
                                .header(accessHeader, BEARER + accessToken))
                //.andExpect(status().isOk()).andReturn();//TODO: 테스트코드 수정 ok에서 변겅
